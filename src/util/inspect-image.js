export default function inspectImage(viewerApi, state, mediaSrc) {
	...
	element.className = 'overflow-hidden fixed top-0 right-0 mr-5 mt-10 text-base bg-gray-200 text-gray-900 rounded-md fixed min-w-5 transition opacity-60 hover:opacity-100';
	...
	element.innerHTML = `
		...
		<div class="overflow-y-scroll px-5 py-5 exif-tags hidden" style="max-height: 500px;">
			${
				['MD5', 'GPSLatitude', 'GPSLongitude', 'Latitude', 'Longitude', 'DateTime', 'DateTimeOriginal', ...Object.keys(exif)]
					.map(tagName => {
